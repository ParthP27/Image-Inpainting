...
class GMCNNModel:
    def __init__(self):
        ...
        # shortcut ops
        self.conv7 = partial(tf.compat.v1.layers.conv2d, kernel_size=7, activation=tf.nn.elu, padding='SAME')
        ...
        self.conv5_ds = partial(tf.compat.v1.layers.conv2d, kernel_size=5, strides=2, activation=tf.nn.leaky_relu, padding='SAME')
    ...
    def build_generator(self, x, mask, reuse=False, name='inpaint_net'):
        ...
        with tf.compat.v1.variable_scope(name, reuse=reuse):
            ...
            x_b1 = tf.compat.v1.image.resize_bilinear(x, [xh, xw], align_corners=True)
            ...
            x = tf.compat.v1.image.resize_nearest_neighbor(x, [xh//2, xw//2], align_corners=True)
            with tf.compat.v1.variable_scope(b_names[1] + 'conv13_upsample'):
            ...
            x_b2 = tf.compat.v1.image.resize_bilinear(x, [xh, xw], align_corners=True)
            ...
            x = tf.compat.v1.image.resize_nearest_neighbor(x, [xh // 2, xw // 2], align_corners=True)
            with tf.compat.v1.variable_scope(b_names[2] + 'conv13_upsample'):
            ...
            x = tf.compat.v1.image.resize_nearest_neighbor(x, [xh, xw], align_corners=True)
            with tf.compat.v1.variable_scope(b_names[2] + 'conv15_upsample'):
